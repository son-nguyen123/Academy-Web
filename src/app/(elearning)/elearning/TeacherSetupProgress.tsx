...
export function TeacherSetupProgress() {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      const advanced = previousCompleted.current !== null && next.completed > previousCompleted.current;
      if (advanced) {
        setExpanded(true);
        if (next.nextStep && `${window.location.pathname}${window.location.search}` !== next.nextStep.href) {
          router.replace(next.nextStep.href);
        }
      }
  ...
  return <div className={styles.teacherSetupOverlay}><section className={expanded ? styles.globalSetupGuide : styles.globalSetupGuideCollapsed} aria-label="Getting started checklist">
    <div className={styles.globalSetupHeader}>
      ...
      <button type="button" onClick={() => setExpanded((value) => !value)} aria-label={expanded ? "Collapse setup guide" : "Expand setup guide"}>{expanded ? <ChevronUp size={17} /> : <ChevronDown size={17} />}</button>
    </div>
